...
filename = [ DD_PROC '/' OUTSTR '_fragility_thr' num2str(THRDENS(tDx)) '_.mat' ] ;
if ~isfile(filename)
...
%     aa = c_aAdj(:,:,idx) ;
    ...
    try
        ...
        [orighub,hubprob,hubarea,hubsus] = ...
            hub_sweep(c,d,'mydef',250,randlevels,eud) ;
...
thr=0.1;
filename = [ DD_PROC '/' OUTSTR '_fragility_thr' num2str(thr) '_.mat' ] ;
load(filename)
...
if VIZIT
    ...
    nice_scatter(ssheet.log10_BrV_,trimmean(hfstack,25),200,grp2idx(ssheet.Order)) 
