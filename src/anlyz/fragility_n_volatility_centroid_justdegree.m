...
filename = [ DD_PROC '/' OUTSTR '_fragilityJustDegree_thr' num2str(THRDENS(tDx)) '_.mat' ] ;
if ~isfile(filename)
...
%     aa = c_aAdj(:,:,idx) ;
    ...
    try
        ...
        [orighub,hubprob,hubarea,hubsus] = ...
            hub_sweep(c,d,'gollo2018',250,randlevels,eud) ;
...
thr=0.1;
filename = [ DD_PROC '/' OUTSTR '_fragilityJustDegree_thr' num2str(thr) '_.mat' ] ;
load(filename)
...
if VIZIT
    ...
    nice_scatter(ssheet.log10_BrV_,trimmean(hfstack,10),200,grp2idx(ssheet.Order)) 
    ...
    nice_scatter(ssheet.log10_BrV_,sum(hfstack<0.33),200,grp2idx(ssheet.Order)) 
    nice_scatter(ssheet.log10_BrV_,sum(hfstack>0.67),200,grp2idx(ssheet.Order)) 
